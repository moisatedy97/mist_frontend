...
const GameName = ({ game }: GameOptionProps): ReactElement => {
    return <p className={"flex justify-start text-2xl font-semibold text-white"}>{game.name}</p>;
};

const GameRating = ({ game }: GameOptionProps): ReactElement => {
    return (
        <div className={"flex items-center gap-1"}>
            <p className={"text-xl font-medium text-white"}>{game.rating}</p>
            <svg
        ...
        return (
            <div className={"flex gap-1"}>
                {game.parent_platforms.map((platforms: Platform, index: number) => {
                    return (
                        <p key={index} className={"text-base font-medium text-white"}>
...
const BuyNowButton = ({ game }: BuyNowButtonProps): ReactElement => {
    ...
    return (
        <button
            className={
                "absolute bottom-5 right-5 rounded-md bg-gradient-to-r from-lime-400 to-lime-600 px-5 py-2 shadow-md hover:from-lime-300 hover:to-lime-500"
            }
            onClick={handleClick}
        >
            <div className={"flex gap-2"}>
                ...
                <p className={"font-medium text-white"}>{"Buy Now"}</p>
            </div>
        </button>
    );
};
